...
| filter-effects ★ | 1,973 / 2,452 | 80.5% |
...
| filter-effects | 833 | 1,973 | +1,140 |
...
- **필터 합성의 리스트 길이 불일치**: 짧은 쪽을 상대 리스트의 항등값으로 채운 뒤 합성한다.
  예전엔 길이가 다르면 합성을 통째로 건너뛰어 언더라이닝(blur(10px))이 결과에서 빠졌다.
...
**되돌린 시도 3(실증)**: box-shadow 보간 결과의 성분 순서를 "길이 → 색"으로 뒤집었더니
box-shadow-interpolation 통과가 282 → 14 로 폭락했다. **색이 먼저**가 맞다(Chrome 계산값
형식). 이 파일의 남은 120 실패는 순서가 아니라 **색 보간 공간**이다 — 기대값이
`oklab(0 0 0)` 인데 우리는 sRGB 에서 보간해 `rgb(0, 0, 0)`/`color(srgb …)` 를 낸다.
§CSS Color 4 는 레거시 sRGB 끼리가 아니면 oklab 에서 보간하도록 정한다. 색 변환기는
이미 있으므로(srgb↔oklab) 보간 경로에 색공간 선택을 붙이는 것이 다음 작업이다.

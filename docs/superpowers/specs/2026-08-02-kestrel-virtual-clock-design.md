...
## 실측 결과와 정정 (2026-08-02, 구현 후)

### 틀린 예측

이 문서는 "1번(가상 시계)만으로 `properties-value-*` 의 `/ values` 서브테스트가 열린다"고
적었다. **틀렸다.** 그 테스트들은 트랜지션을 인라인 style 쓰기가 아니라 **클래스 토글과
시트 주입**으로 유발한다. 당시 트랜지션 생성이 `el.style` 쓰기 훅에만 걸려 있었으므로
시계만으로는 아무것도 열리지 않았다(측정: 통과 886 → 886, 완전 동일).

### 측정 방법 정정 (중요)

WPT 통과 수가 실행마다 흔들리는 원인은 러너 병렬성이 아니라 **JS 실행 예산이 벽시계
기준**(`SCRIPT_BUDGET_MS=5s`, `TOTAL_BUDGET_MS=10s`)이기 때문이다. 무거운 파일이 중간에
잘리고, 잘리는 지점이 머신 부하에 따라 달라진다. 같은 바이너리로 같은 파일이 344~406
서브테스트를 오갔다. `css/motion` 통과가 1244→1206 으로 보였던 "회귀"는 전부 이 아티팩트
였다(기능을 전부 꺼도 동일하게 잘리는 것으로 확인). 두 예산 모두 환경변수로 조절되므로
비교 측정은 반드시 넉넉한 예산(30s/60s)에서 한다. **분모와 "하네스 못 돈 파일" 수를 함께
봐야 한다 — 분모가 줄면서 %가 오르는 건 개선이 아니라 잘린 것이다.**

### 실제로 필요했던 것 (구현함)

1. 트랜지션 생성을 스타일 재계산 비교로 이동(§CSS Transitions 4 스타일 변경 이벤트).
2. `transition-property` 목록에서 **뒤 항목 우선**, `all` 과 개별 이름의 순서 경쟁.
3. 계산값이 `currentcolor` 인 프로퍼티는 `color` 만 바뀔 때 전이하지 않음.
4. transitionrun/start/end 발화 + 시계가 트랜지션 경계에서 멈추게.
5. `document.getAnimations()`/`element.getAnimations()` 실구현(+`effect.target`).
   WPT 트랜지션 하네스는 이걸로 슬라이스를 구동한다 — 없으면 TypeError 로 즉사했다.
6. 시계 전진 뒤 마이크로태스크 드레인, 그리고 전진을 `enter_js` 창 안에서 수행
   (밖이면 이벤트 핸들러가 `document` 를 못 본다).
7. 끝난 트랜지션 제거(안 치우면 경계/이벤트 스캔이 제곱으로 든다).
8. 엔진 일반 버그: private 이름을 `"#x"` 문자열로 표현해 `o["#sel"] = v` 같은 평범한
   대입이 private 필드 쓰기로 오인됐다. 렉서가 NUL 접두를 붙여 문법으로만 만들어지게 했다.

### 결과

`properties-value-001.html` 은 560 서브테스트가 **전부 미실행(NOTRUN)** 이던 상태에서
실제 실행으로 바뀌었다. 테스트 수를 줄여 재면 50개 중 45개가 통과한다(2개→1/2, 10개→5/10,
50개→45/100). 전체 280개를 한 파일에서 돌리면 예산을 넘긴다.

### 다음 병목 (측정됨)

`sample` 프로파일 기준 최대 핫스팟은 **`collect_computed_styles`** 와 그 안의 해시/문자열
할당이다. `getComputedStyle` 한 번이 문서 전체를 재스타일하고 **모든 요소**의
`HashMap<String, String>`(프로퍼티 약 200개)을 새로 만든다. 강제 레이아웃을 반복하는
페이지에서 이게 제곱으로 든다. 계산 스타일을 **필요한 요소만 지연 생성**하도록 바꾸는 것이
다음 성능 작업이며, 트랜지션뿐 아니라 JS 중심 페이지 전반에 효과가 있다.

남은 애니메이션 서브프로젝트는 문서 앞부분의 2번(Web Animations 객체 모델 — `Animation`
상태기계, 다중 키프레임, 효과 스택)과 4번(보간 타입 레지스트리)이다.

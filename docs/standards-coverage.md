...
- 시각: 배경색, border(단축/변별/다색), border-radius(AA), box-shadow(SDF),
  리스트 마커(list-style-type).
- 이미지: png/jpeg(baseline). 폼 컨트롤 UA 기본 외형(캐스케이드로 저작자 CSS 우선).
- 폰트: 자체 TrueType/CFF 렌더 + 글리프 캐시 (라틴+한글).
...
- **상속**: → color/font-size/text-align. ✗ 나머지(font-family/weight/line-height/
  white-space 등).
- **폼 컨트롤**: → UA 외형 + 값 텍스트. ✗ 실제 위젯(체크박스/라디오/드롭다운),
  입력/포커스/제출.
- **폰트 커버리지**: → 라틴+한글. ✗ CJK(일/중)·아랍(두부 □), 볼드/이탤릭 렌더.

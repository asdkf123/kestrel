...
- [~] **지연 제너레이터** — 재개가능 인터프리터(generator.rs). 무한 제너레이터/양방향 `next(v)`/`yield*` 위임/`return`값/`it.return`·`it.throw`/try 재개. 깊은 식 위치 yield(`a+(yield b)`)는 후속(명확한 에러)
- [ ] **Symbol 타입** (typeof/계산 키), ES 모듈 그래프, BigInt, Intl, 태그드 템플릿
...
1. **Symbol 타입** (typeof/계산 키/사용자 이터러블) + 지연 제너레이터 깊은 식 위치 yield — 모던 JS 앱의 근간

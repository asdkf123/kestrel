...
12. DOM API 확충 + 이벤트 모델. (DOM/이벤트/타이머 상당수 + className/id 세터 ✓)
13. JS DOM 변경 → 리플로우. 로드 시 async 변경은 반영 ✓ (fetch→DOM→렌더 확인).
    타이머/이벤트 콜백 **후** 리플로우(로드 후 SPA 갱신)는 남음.
14. ~~fetch~~ ✓, ~~Promise/마이크로태스크~~ ✓ (90d8793). XHR/타이머 후 드레인/rAF 남음.
15. ~~async/await~~ ✓ (6f4ef62). 정규식 엔진/일부 빌트인은 남음. 클래스/클로저 지원.

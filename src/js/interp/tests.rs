...
fn more_array_string_methods() {
    ...
    // 배열의 비인덱스 프로퍼티도 서술자 속성을 추적한다(prop_attrs): defineProperty
    // {value} 만 주면 default-false, 재정의 검증, freeze 로 non-writable/non-configurable.
    assert_eq!(
        run_str("var a=[]; Object.defineProperty(a,'p',{value:12}); var d=Object.getOwnPropertyDescriptor(a,'p'); d.writable+','+d.enumerable+','+d.configurable"),
        "false,false,false"
    );
    assert!(run_bool(
        "var a=[]; Object.defineProperty(a,'p',{value:12}); try{ Object.defineProperty(a,'p',{value:36}); false }catch(e){ e instanceof TypeError }"
    ));
    assert!(run_bool(
...
fn function_names_follow_standard() {
    ...
    // 심볼 키 프로퍼티/메서드의 이름은 "[desc]"/""/"[Symbol.x]" (내부 키 아님, §10.2.9)
    assert_eq!(run_str("var s=Symbol('test262'); var o={[s]:()=>{}}; o[s].name"), "[test262]");
    assert_eq!(run_str("var s=Symbol(); var o={[s]:()=>{}}; o[s].name"), "");
...
fn array_set_length_stops_at_nonconfigurable() {
    ...
    // defineProperty length 경로: non-configurable 인덱스에 막히면 그 위에서 멈추고
    // TypeError 를 던진다(§10.4.2.4, defineProperty Throw=true).
    assert!(run_bool(
        "var c=[1,2,3,4]; Object.defineProperty(c,'1',{value:8,configurable:false}); \
         var threw=false; try{ Object.defineProperty(c,'length',{value:0}); }catch(e){ threw = e instanceof TypeError; } \
         threw && c.length===2"
    ));
    // 정상 축소 무회귀.

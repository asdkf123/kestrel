...
var __kTAProto = {
  ...
  includes: function(v, fromIndex){ __kTAValidate(this); var len=this.length; var n=(fromIndex===undefined)?0:Math.trunc(+fromIndex); var k = n>=0 ? n : Math.max(0, len+n); for (var i = k; i < len; i++){ var x=this[i]; if (x===v || (x!==x && v!==v)){ return true; } } return false; },
  ...
  lastIndexOf: function(v, fromIndex){ __kTAValidate(this); var len=this.length; var i = (fromIndex===undefined) ? len-1 : Math.trunc(+fromIndex); if (i<0){ i+=len; } if(i>=len){ i=len-1; } for (; i>=0; i--){ if (this[i]===v){ return i; } } return -1; },
...
if (!window.Iterator) {
  ...
  // §27.1.4: 콜백이 callable 아니면 밑 이터레이터를 닫고 TypeError(IteratorClose).
  var __kIterFnC = function(v, it){ if (typeof v !== 'function') { __kIterClose(it); throw new TypeError('argument is not a function'); } };
  // take/drop 의 limit: ToNumber 후 NaN/음수면 밑 이터레이터를 닫고 RangeError.
  var __kIterLim = function(n, it){ var x = +n; if (x !== x) { __kIterClose(it); throw new RangeError('limit must not be NaN'); } x = Math.trunc(x); if (!(x >= 0)) { __kIterClose(it); throw new RangeError('limit must be a non-negative number'); } return x; };
  ...
  __kIterProto.map = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; return __kIterHelper(it, function(){ return (function*(){ while(true){ var r=__kIterStep(nx,it); if(r.done) return; var m; try{ m=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } yield m; } })(); }); };
  __kIterProto.filter = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; return __kIterHelper(it, function(){ return (function*(){ while(true){ var r=__kIterStep(nx,it); if(r.done) return; var k; try{ k=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } if(k) yield r.value; } })(); }); };
  __kIterProto.take = function(n){ 'use strict'; __kIterRecv(this); var lim=__kIterLim(n, this); var it=this,nx=it.next; return __kIterHelper(it, function(){ return (function*(){ var c=0; if(c>=lim){ __kIterClose(it); return; } while(true){ var r=__kIterStep(nx,it); if(r.done) return; yield r.value; c++; if(c>=lim){ __kIterClose(it); return; } } })(); }); };
  __kIterProto.drop = function(n){ 'use strict'; __kIterRecv(this); var lim=__kIterLim(n, this); var it=this,nx=it.next; return __kIterHelper(it, function(){ return (function*(){ var c=0; while(c<lim){ var r=__kIterStep(nx,it); if(r.done) return; c++; } while(true){ var r2=__kIterStep(nx,it); if(r2.done) return; yield r2.value; } })(); }); };
  __kIterProto.flatMap = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; return __kIterHelper(it, function(){ return (function*(){ while(true){ var r=__kIterStep(nx,it); if(r.done) return; var inner; try{ inner=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } var iit=(inner!=null && inner[Symbol.iterator])?inner[Symbol.iterator]():inner; while(true){ var ir=iit.next(); if(ir.done) break; yield ir.value; } } })(); }); };
  __kIterProto.reduce = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0,acc,has=arguments.length>1; if(has) acc=arguments[1]; while(true){ var r=__kIterStep(nx,it); if(r.done) break; if(!has){ acc=r.value; has=true; continue; } acc=fn(acc,r.value,i); i++; } if(!has) throw new TypeError('Reduce of empty iterator with no initial value'); return acc; };
  __kIterProto.toArray = function(){ 'use strict'; __kIterRecv(this); var it=this,nx=it.next,out=[]; while(true){ var r=__kIterStep(nx,it); if(r.done) break; out.push(r.value); } return out; };
  __kIterProto.forEach = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; while(true){ var r=__kIterStep(nx,it); if(r.done) break; fn(r.value,i++); } return undefined; };
  __kIterProto.some = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; while(true){ var r=__kIterStep(nx,it); if(r.done) return false; var m; try{ m=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } if(m){ __kIterClose(it); return true; } } };
  __kIterProto.every = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; while(true){ var r=__kIterStep(nx,it); if(r.done) return true; var m; try{ m=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } if(!m){ __kIterClose(it); return false; } } };
  __kIterProto.find = function(fn){ 'use strict'; __kIterRecv(this); __kIterFnC(fn, this); var it=this,nx=it.next,i=0; while(true){ var r=__kIterStep(nx,it); if(r.done) return undefined; var m; try{ m=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } if(m){ __kIterClose(it); return r.value; } } };
...
// 프려류로 폴리필한 내장 메서드는 [[Construct]] 가 없고(§ 내장 함수), name/length 도
// 스펙 값을 가져야 한다. obj.m=function(){} 대입은 익명이라 name="" 이고, 내부 마커
// '\u0000nonctor' 를 달아 비생성자로 표시한다(is_constructor/new 가 본다). 내부 키라
// 열거엔 안 보인다. 프려류드 맨 끝에서 실행해 늦게 정의된 메서드(Promise 등)도 잡는다.
(function(){
  function fix(holder, specs){
    if (!holder) return;
    for (var n in specs){
      var f = holder[n];
      if (typeof f !== 'function') continue;
      try { Object.defineProperty(f, '\u0000nonctor', { value: true, writable: false, enumerable: false, configurable: false }); } catch (e) {}
      try { Object.defineProperty(f, 'name', { value: n, writable: false, enumerable: false, configurable: true }); } catch (e) {}
      var L = specs[n];
      if (L >= 0) { try { Object.defineProperty(f, 'length', { value: L, writable: false, enumerable: false, configurable: true }); } catch (e) {} }
    }
  }
  fix(Array.prototype, { copyWithin: 2, lastIndexOf: 1, toReversed: 0, toSorted: 1, findLast: 1, findLastIndex: 1 });
  fix(Object, { is: 2, hasOwn: 2, getOwnPropertyDescriptors: 1, fromEntries: 1 });
  fix(typeof Promise !== 'undefined' ? Promise : null, { all: 1, race: 1, any: 1, allSettled: 1 });
  fix(String.prototype, { normalize: 0 });
})();
"#;

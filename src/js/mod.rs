...
if (!window.Iterator) {
  ...
  __kIterProto.some = function(fn){ __kIterRecv(this); __kIterFn(fn); var it=this,nx=it.next,i=0; while(true){ var r=__kIterStep(nx,it); if(r.done) return false; var m; try{ m=fn(r.value,i++); }catch(e){ __kIterClose(it); throw e; } if(m){ __kIterClose(it); return true; } } };

...
use crate::paint_svg::emit_svg;
pub use crate::paint_svg::{rasterize_svg, svg_natural_size};
...
impl Canvas {
    ...
    pub(crate) fn new_layer(width: usize, height: usize) -> Canvas {
        Canvas {
...
pub(crate) fn flatten_path(d: &str) -> Vec<Vec<(f32, f32)>> {
    // 1) 명령 그룹으로 토큰화
    struct Group {
        cmd: char,
        nums: Vec<f32>,
...
pub(crate) fn stroke_line_quad(x1: f32, y1: f32, x2: f32, y2: f32, sw: f32) -> Option<Vec<(f32, f32)>> {
...
pub(crate) fn find_svg_by_id<'a, 'b>(

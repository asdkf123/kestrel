// HTML 명명 문자 참조 표 (HTML 표준 §13.5 "Named character references").
// **기계 생성**: https://html.spec.whatwg.org/entities.json 에서 추출했다.
// 손으로 고른 40개만 있던 예전 표는 나머지 2,191개를 조용히 원문 그대로 남겼다.
// 세미콜론이 없는 레거시 형태(&amp 등 106개)도 표준이 인정하므로 그대로 담는다.
// 이름 순 정렬 — 최장 일치 탐색에 쓴다.
pub static NAMED_REFS: &[(&str, &str)] = &[
    ("AElig", "\u{00C6}"),
    ("AElig;", "\u{00C6}"),
    ("AMP", "\u{0026}"),
    ("AMP;", "\u{0026}"),
